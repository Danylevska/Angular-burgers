...
confirmOrder() {
  if (this.form.valid) {
    alert("Thank you for the order. We will connect you as soon as possible.");
    this.form.reset();
  }
}
...
// Чтобы выложить на хостинг в angular.json
// в "outputHashing": "all",  дописываем:
// "baseHref": "/burgers/"

//Меняем данные по ограничениям размера сайта
// {
//   "type": "initial",
//   "maximumWarning": "1mb",
//   "maximumError": "1mb"
...
// Делаем сборку:
//Остановить проект Cntr + C
//дальше npx ng build

// Появилась папка dist
// в ней папка burgers (или burgers/browser) которая содержит все что нужно выложить на хостинг (перетягивать только содержимое папки)

// Чтобы выложить сайт на хостинг Гитхаб:
//Settings => Pages => Branch и выбираем верку main (по умолчанию была None)
// затем Save

// в разделе Action можно отследить процесс публикации сайта

// после полной загрузки возвращаемся на вкладку Code
//с права в Enviroment нажать github-pages (должно быть Active)

// и дальше нажать на View deployment
